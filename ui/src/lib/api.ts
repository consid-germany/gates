...
export async function toggleGateState(gate: Gate): Promise<Gate> {
    const { data: updatedGate, error } = await client.PUT("/gates/{group}/{service}/{environment}/state", {
        params: {
            path: {
                group: gate.group,
...
export async function addCommentToGate(gate: Gate, message: string): Promise<Gate> {
    const { data: updatedGate, error } = await client.POST("/gates/{group}/{service}/{environment}/comments", {
        params: {
            path: {
                group: gate.group,
                service: gate.service,
...
export async function removeCommentFromGate(gate: Gate, commentId: string): Promise<Gate> {
    const { data: updatedGate, error } = await client.DELETE("/gates/{group}/{service}/{environment}/comments/{comment_id}", {
        params: {
            path: {
                group: gate.group,
                service: gate.service,
